    ...
    function Panel(props) {
      ...
      const submit = () => {
        ...
        if (link === '' && pasted === '' && filePath === '') {
          setError('请填写链接、粘贴文本，或提供文件路径（三者其一）')
          return
        }
        if (typeof submitDeepread !== 'function') {
          setError('精读提交通道不可用，请直接对对话说：请用 deepread 精读 <内容>')
          return
        }
        let target = null
        if (pasted !== '') target = '正文如下：\n' + pasted
        else if (/^https?:\/\//i.test(link) || link.startsWith('mp.weixin.qq.com') || link.includes('weixin.qq.com')) target = '链接：' + link
        else if (filePath !== '') target = '文件路径：' + filePath
