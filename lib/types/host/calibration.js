...
export function createCalibrationRuntime(ctx, tune) {
    const state = { rateTokPerSec: null, latencyMs: null, calls: 0, updatedAt: null, loaded: false };
    const { getTable } = createOptionalStorageTable(ctx, statsDomainSpec, (domain) => domain.table('stats'));
    ...
    function modelRateDefaults() {
        ...
        for (const candidate of MODEL_RATE_DEFAULTS) {
            if (candidate.match.test(id))
                return { rate: candidate.rate, latency: candidate.latency };
        }
    ...
    function recordCalibration(rateTokPerSec, latencyMs) {
        if (!Number.isFinite(rateTokPerSec) || rateTokPerSec <= 0)
            return;
