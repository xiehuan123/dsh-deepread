...
export function apply(ctx, config) {
    if (!isHostContext(ctx))
        throw new TypeError('deepread requires a valid Harness Host context');
...
function applyHost(ctx, config) {
    ...
    function sectionSystem(depth, language, focus) {
        ...
        let sys = '你是一位专业的精读分析师，擅长从书籍和文章中提取核心观点、论证结构与关键论据。\n'
            ...
            + '输出语言：' + lang + '。\n';
        if (focus.trim() !== '')
            sys += '读者特别关注：' + focus.trim() + '。\n';
        if (depth === 'quick')
            sys += '模式：快速抓要点——arguments 不超过 3 条，quotes 不超过 3 条，concepts 不超过 4 个，questions 不超过 3 个。';
        else
            sys += '模式：深度精读——论据要具体，尽可能多地引用原文。';
        return sys;
    }
    function sectionUser(text, index, total) {
        if (total > 1)
            return '【第 ' + (index + 1) + ' / ' + total + ' 部分】\n\n' + text;
        return '以下是待精读的内容：\n\n' + text;
    }
    const FINAL_SCHEMA = [
        '{',
    ...
    async function batchEstimateFlow(args, language) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (text.length > tune.maxInputChars)
                text = text.slice(0, tune.maxInputChars);
            totalChars += text.length;
            const q = buildEstimate(text, 'quick').modes.find((mm) => mm.mode === 'quick');
            if (q === undefined)
                throw new Error('预算估算缺少 quick 模式');
            totalCalls += q.calls;
    ...
    async function batchFlow(args, language, signal, onProgress) {
        ...
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            if (progress !== null)
                progress('解析第 ' + (i + 1) + '/' + items.length + ' 篇…');
            ...
            if (progress !== null)
                progress('精读第 ' + (i + 1) + '/' + items.length + ' 篇…');
            ...
            if (progress !== null)
                progress('完成第 ' + (i + 1) + '/' + items.length + ' 篇');
        }
    ...
    async function computeResult(input, opts = {}) {
        ...
        if (depth === 'map') {
            ...
            for (let i = 0; i < parts.length; i++) {
                if (onProgress !== null)
                    onProgress('精读第 ' + (i + 1) + '/' + parts.length + ' 段…');
                ...
                if (onProgress !== null)
                    onProgress('完成第 ' + (i + 1) + '/' + parts.length + ' 段');
            }
    ...
    const registerPackagedSkill = (skillCtx) => {
        if (skillCtx.skills === undefined)
            return;
    ...
    const registerBudgetRoute = (routeCtx) => {
        const panelWebServer = routeCtx.webServer ?? routeCtx.get('webServer');
        if (panelWebServer === undefined || typeof panelWebServer.register !== 'function')
            return;
        try {
            routeCtx.effect(() => panelWebServer.register({
                kind: 'exact',
            ...
            // The budget API is Web-only; route registry failures must not block the Host tool.
        }
    };
