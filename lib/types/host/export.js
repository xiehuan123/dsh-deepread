...
const EMPTY_RECORD = {};
export function createExportTools(ctx, tune, tools) {
    ...
    function cacheLabel(meta) {
        ...
        if (meta.cache === 'hit')
            return '缓存命中（抓取于 ' + fetched + '，未重新联网）';
        if (meta.cache === 'fallback')
            return '回退缓存（抓取于 ' + fetched + '）';
        if (meta.cache === 'miss')
            return '已重新抓取并写入缓存';
        if (meta.cache === 'disabled')
            return '缓存已禁用';
        return '';
    }
    function metaFooter(meta) {
        ...
        if (est !== null && Array.isArray(est.modes)) {
            ...
            if (row !== null && typeof row.calls === 'number')
                estText = ' · 本次预算：约 ' + row.calls + ' 次调用 / ' + row.totalTokens + ' token / ≈' + row.minutes + ' 分钟';
        }
        let execText = '';
        ...
        if (isRecord(st)) {
            ...
            if (typeof meta.chunks === 'number' && meta.chunks > 0)
                bits.push('分段 ' + meta.chunks);
            if (typeof st.resolveMs === 'number')
                bits.push('抓取 ' + fmtSec(st.resolveMs) + 's');
            if (typeof st.extractMs === 'number' && st.extractMs > 0)
                bits.push('解析 ' + fmtSec(st.extractMs) + 's');
            if (typeof st.calls === 'number')
                bits.push('模型 ' + st.calls + ' 次 ' + fmtSec(st.llmMs) + 's');
    ...
    function renderFeynmanMarkdown(v) {
        ...
        if (str(v.summary, '') !== '')
            lines.push('', '**一句话总结**：' + v.summary);
        if (str(v.thesis, '') !== '')
            lines.push('', '**核心论点**：' + v.thesis);
        ...
        arr(v.feynmanChapters).forEach((ch) => {
            ...
            if (str(o.chapterMap, '') !== '')
                lines.push('', '**章节导图**：', '```mermaid', str(o.chapterMap, ''), '```');
            if (str(o.explanation, '') !== '')
                lines.push('', '**费曼讲解（合上书）**：', str(o.explanation, ''));
            const gaps = arr(o.gaps);
            if (gaps.length > 0)
                lines.push('', '**知识缺口**：', gaps.map((g) => '- ' + g).join('\n'));
            const fixes = arr(o.corrections);
            if (fixes.length > 0)
                lines.push('', '**原文修正**：', fixes.map((f) => '- ' + f).join('\n'));
        });
        if (str(v.bookMap, '') !== '')
            lines.push('', '## 合并全书导图', '```mermaid', str(v.bookMap, ''), '```');
        if (str(v.finalExplanation, '') !== '')
            lines.push('', '## 再讲一次（全书终讲）', str(v.finalExplanation, ''));
    ...
    function renderMapMarkdown(v) {
        ...
        if (str(v.coreQuestion, '') !== '')
            lines.push('', '**核心问题**：' + v.coreQuestion);
        ...
        if (items.length > 0) {
            ...
            items.forEach((it, i) => {
                ...
                arr(o.relations).forEach((r) => {
                    ...
                    if (str(ro.to, '') !== '')
                        lines.push('   ↳ ' + str(ro.type, '支持') + ' → ' + ro.to);
                });
            });
        }
        ...
        if (str(v.mermaid, '') !== '')
            lines.push('', '**Mermaid 思维导图**：', '```mermaid', str(v.mermaid, ''), '```');
        if (str(v.xmindOutline, '') !== '')
            lines.push('', '**XMind 大纲**：', '```markdown', str(v.xmindOutline, ''), '```');
    ...
    function renderEstimateMarkdown(v) {
        ...
        if (est.batch === true) {
            ...
            if (est.calibrated === true)
                lines.push('', '> 已使用运行时实测校准速率（' + tps + ' tok/s / ' + lat + 'ms）。');
        }
        else {
            ...
            if (typeof est.chars === 'number')
                lines.push('', '输入字数：' + est.chars + '（超过 ' + tune.maxInputChars + ' 会被截断）');
            if (est.sampled === true)
                lines.push('', '> 本预检采用 PDF 采样外推（前 2 页字数 ÷ 2 × 总页数），仅作数量级参考。');
            if (est.calibrated === true)
                lines.push('', '> 已使用运行时实测校准速率（' + tps + ' tok/s / ' + lat + 'ms）。');
    ...
    function renderBatchMarkdown(v) {
        ...
        if (items.length > 0) {
            ...
            items.forEach((it) => {
                ...
                if (str(io.thesis, '') !== '')
                    lines.push('- 核心论点：' + io.thesis);
                if (str(io.summary, '') !== '')
                    lines.push('- 摘要：' + io.summary);
            });
        }
        ...
        if (str(cmp.complementarity, '') !== '')
            lines.push('', '## 互补关系', str(cmp.complementarity, ''));
    ...
    function renderMarkdown(value) {
        const v = isRecord(value) ? value : EMPTY_RECORD;
        if (v.kind === 'background')
            return '已启动后台精读任务 ' + str(v.jobId, '') + '（' + str(v.label, '') + '）。\n用 job_output 读取进度与最终报告；job_kill 可取消。';
        if (v.kind === 'map')
            return renderMapMarkdown(v);
        if (v.kind === 'feynman')
            return renderFeynmanMarkdown(v);
        if (v.kind === 'estimate')
            return renderEstimateMarkdown(v);
        if (v.kind === 'batch')
            return renderBatchMarkdown(v);
        ...
        if (arr(v.structure).length > 0)
            lines.push('', '**论证脉络**：' + arr(v.structure).join(' → '));
        const cons = sanitizeConcepts(v.concepts);
        if (cons.length > 0)
            lines.push('', '**核心概念**：' + cons.map((c) => c.term + (c.explanation !== '' ? '（' + c.explanation + '）' : '')).join('；'));
    ...
    function buildMindTree(value) {
        ...
        if (v.kind === 'map') {
            const kids = [];
            if (typeof v.coreQuestion === 'string' && v.coreQuestion !== '')
                kids.push(node('核心问题', [node(v.coreQuestion)]));
            const cons = Array.isArray(v.coreConclusions) ? v.coreConclusions : [];
            if (cons.length > 0)
                kids.push(node('核心结论', cons.map((c) => node(String(c)))));
            ...
            if (caveats.length > 0)
                kids.push(node('反对意见与局限', caveats.map((c) => node(String(c).slice(0, 60)))));
            const rqs = Array.isArray(v.recallQuestions) ? v.recallQuestions : [];
            if (rqs.length > 0)
                kids.push(node('主动回忆问题', rqs.map((q) => node(String(q).slice(0, 60)))));
        ...
        if (v.kind === 'feynman') {
            const kids = [];
            if (arr(v.toc).length > 0)
                kids.push(node('目录', arr(v.toc).map((t) => node(String(t).slice(0, 50)))));
            const qs = arr(v.questions);
            if (qs.length > 0)
                kids.push(node('阅读问题', qs.map((q) => node(String(q).slice(0, 50)))));
        ...
        const kids = [];
        if (typeof v.thesis === 'string' && v.thesis !== '')
            kids.push(node('核心论点', [node(v.thesis)]));
        ...
        if (quotes.length > 0)
            kids.push(node('金句摘录', quotes.map((q) => {
    ...
    function htmlTree(n, depth) {
        ...
        const kids = n.children !== undefined && n.children.attached !== undefined ? n.children.attached : [];
        if (kids.length > 0)
            h += '<ul>' + kids.map((k) => htmlTree(k, depth + 1)).join('') + '</ul>';
        return h + '</li>';
    }
    function buildHtml(value) {
        ...
        const depthLabels = { quick: '快速要点', deep: '深度精读', book: '全书精读', map: '知识地图', feynman: '费曼读书法' };
        ...
        if (isMap) {
            if (typeof v.summary === 'string' && v.summary !== '')
                add('摘要', '<p class="lead">' + esc(v.summary) + '</p>');
            if (typeof v.coreQuestion === 'string' && v.coreQuestion !== '')
                add('核心问题', '<div class="hl"><span class="hl-label">作者试图回答</span>' + esc(v.coreQuestion) + '</div>');
            const cons = Array.isArray(v.coreConclusions) ? v.coreConclusions : [];
            if (cons.length > 0)
                add('核心结论', '<ol class="concl">' + cons.map((c) => '<li>' + esc(c) + '</li>').join('') + '</ol>');
            const items = Array.isArray(v.items) ? v.items : [];
            const groups = {};
            for (const it of items) {
                const o = isRecord(it) ? it : { claim: String(it) };
            ...
            for (const t of ['核心结论', '分论点', '原因或作用机制', '事实', '数据', '案例', '隐含前提', '反对意见', '限制条件', '可执行建议']) {
                ...
                h += g.map((o, i) => {
                    ...
                    if (ev !== '')
                        item += '<div class="ev' + (ev === '原文未提供证据' ? ' ev-missing' : '') + '">证据 · ' + esc(ev) + '</div>';
                    const tags = [];
                    if (typeof o.source === 'string' && o.source !== '')
                        tags.push('<span class="chip plain">位置 ' + esc(o.source) + '</span>');
                    if (cc !== '')
                        tags.push('<span class="chip ' + confClass(cc) + '">' + esc(cc) + '</span>');
                    if (tags.length > 0)
                        item += '<div class="tags">' + tags.join('') + '</div>';
                    rels.forEach((r) => {
                        const ro = isRecord(r) ? r : EMPTY_RECORD;
                        if (typeof ro.to === 'string' && ro.to !== '')
                            item += '<div class="rel">' + esc(typeof ro.type === 'string' ? ro.type : '支持') + ' → ' + esc(ro.to) + '</div>';
                    });
                    return item + '</div>';
                }).join('');
                add(t, h);
            ...
            if (dps.length > 0) {
                add('关键数据', '<div class="dgrid">' + dps.map((d) => {
                    const o = isRecord(d) ? d : { value: String(d) };
                    const bits = [];
                    if (typeof o.period === 'string' && o.period !== '')
                        bits.push('<span>时间 ' + esc(o.period) + '</span>');
                    if (typeof o.subject === 'string' && o.subject !== '')
                        bits.push('<span>对象 ' + esc(o.subject) + '</span>');
                    if (typeof o.baseline === 'string' && o.baseline !== '')
                        bits.push('<span>基准 ' + esc(o.baseline) + '</span>');
                    if (typeof o.source === 'string' && o.source !== '')
                        bits.push('<span>来源 ' + esc(o.source) + '</span>');
                    if (typeof o.location === 'string' && o.location !== '')
                        bits.push('<span>位置 ' + esc(o.location) + '</span>');
            ...
            const caveats = Array.isArray(v.caveats) ? v.caveats : [];
            if (caveats.length > 0)
                add('反对意见与局限', '<ul class="list">' + caveats.map((c) => '<li>' + esc(c) + '</li>').join('') + '</ul>');
            if (typeof v.mermaid === 'string' && v.mermaid !== '')
                add('Mermaid 思维导图', '<pre class="pre">' + esc(v.mermaid) + '</pre><p class="note">复制到 mermaid.live 或支持 Mermaid 的编辑器渲染</p>');
            if (typeof v.xmindOutline === 'string' && v.xmindOutline !== '')
                add('XMind 大纲', '<pre class="pre">' + esc(v.xmindOutline) + '</pre>');
            const rqs = Array.isArray(v.recallQuestions) ? v.recallQuestions : [];
            if (rqs.length > 0)
                add('主动回忆问题', '<ol class="list">' + rqs.map((q) => '<li>' + esc(q) + '</li>').join('') + '</ol>');
        }
        else if (isFeynman) {
            if (typeof v.summary === 'string' && v.summary !== '')
                add('一句话总结', '<p class="lead">' + esc(v.summary) + '</p>');
            if (typeof v.thesis === 'string' && v.thesis !== '')
                add('核心论点', '<div class="hl">' + esc(v.thesis) + '</div>');
            const toc = Array.isArray(v.toc) ? v.toc : [];
            if (toc.length > 0)
                add('浏览目录', '<ol class="list">' + toc.map((t) => '<li>' + esc(t) + '</li>').join('') + '</ol>');
            const qs = Array.isArray(v.questions) ? v.questions : [];
            if (qs.length > 0)
                add('阅读问题清单', '<ol class="list">' + qs.map((q) => '<li>' + esc(q) + '</li>').join('') + '</ol>');
            ...
            for (const c of chs) {
                ...
                if (typeof o.chapterMap === 'string' && o.chapterMap !== '')
                    h += '<pre class="pre">' + esc(o.chapterMap) + '</pre>';
                if (typeof o.explanation === 'string' && o.explanation !== '')
                    h += '<div class="hl"><span class="hl-label">合上书讲解</span>' + esc(o.explanation) + '</div>';
                const gaps = Array.isArray(o.gaps) ? o.gaps : [];
                if (gaps.length > 0)
                    h += '<p class="ev-missing">知识缺口：' + gaps.map((g) => esc(g)).join('；') + '</p>';
                const fixes = Array.isArray(o.corrections) ? o.corrections : [];
                if (fixes.length > 0)
                    h += '<p class="ev">原文修正：' + fixes.map((f) => esc(f)).join('；') + '</p>';
                add('第 ' + o.index + ' 章 · ' + (typeof o.title === 'string' ? o.title : ''), h);
            }
            if (typeof v.bookMap === 'string' && v.bookMap !== '')
                add('合并全书导图', '<pre class="pre">' + esc(v.bookMap) + '</pre>');
            if (typeof v.finalExplanation === 'string' && v.finalExplanation !== '')
                add('再讲一次（全书终讲）', '<div class="hl">' + esc(v.finalExplanation) + '</div>');
            ...
            const args = Array.isArray(v.arguments) ? v.arguments : [];
            if (args.length > 0)
                add('论证结构', '<ol class="concl">' + args.map((a) => {
                    const o = isRecord(a) ? a : { claim: String(a) };
            ...
            const quotes = Array.isArray(v.quotes) ? v.quotes : [];
            if (quotes.length > 0)
                add('金句摘录', '<ul class="list">' + quotes.map((q) => {
                    const o = isRecord(q) ? q : { text: String(q) };
            ...
            if (concepts.length > 0)
                add('核心概念', '<ul class="list">' + concepts.map((c) => {
            ...
            const chapters = Array.isArray(v.chapters) ? v.chapters : [];
            if (chapters.length > 0)
                add('章节脉络', '<ol class="list">' + chapters.map((c) => {
                    const o = isRecord(c) ? c : EMPTY_RECORD;
            ...
            if (questions.length > 0)
                add('批判性思考', '<ul class="list">' + questions.map((q) => '<li>' + esc(q) + '</li>').join('') + '</ul>');
        }
    ...
    async function attachExports(value, exportFmt) {
        ...
        try {
            ...
            const files = {};
            if (want.md)
                files.md = await writeOne(folder + '/' + safe + '.md', markdown);
            if (want.mm)
                files.mm = await writeOne(folder + '/' + safe + '.mm', buildFreeMind(value));
            if (want.html)
                files.html = await writeOne(folder + '/' + safe + '.html', buildHtml(value));
